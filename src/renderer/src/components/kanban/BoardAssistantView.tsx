...
export function BoardAssistantView({
  ...
  const handleCreateDrafts = useCallback(
    async (onlySelected: boolean) => {
      ...
      try {
          ...
          try {
            ...
            const message =
              reloadError instanceof Error
                ? reloadError.message
                : 'Failed to reload created tickets.'
        ...
        if (failures.length > 0) {
          ...
          addLocalSystemMessage(
            createdCount > 0
              ? `Some selected tickets were created, but ${message}`
              : `Failed to create selected tickets: ${message}`
          )
          toast.error(message)
          return
        }
        ...
        setStatus('idle')
        navigateToBoard()
        toast.success(`Created ${createdCount} ticket${createdCount === 1 ? '' : 's'}.`)
      } catch (error) {
        const message =
    ...
    [addLocalSystemMessage, drafts, markDraftsCreated, navigateToBoard, setError, setStatus]
  )
