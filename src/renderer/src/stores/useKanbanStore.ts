    ...
    (set, get) => ({
      ...
      loadTickets: async (projectId: string) => {
        ...
        try {
          ...
          const diagnostics = await kanban.diagnostics.get(projectId).catch(() => [])
          set((state) => {
      ...
      createTicket: async (projectId: string, data: KanbanTicketCreate) => {
        const ticket = await kanban.ticket.create(projectId, data)
        set((state) => {
          const next = new Map(state.tickets)
            ...
            switch (event.type) {
              case 'session_completed': {
                ...
                if (event.tokenDelta && event.tokenDelta > 0) {
                  kanban.ticket
                    .addTokens(projectId, ticket.id, event.tokenDelta)
                    .then((updated) => {
                      if (updated) {
                        set((state) => {
                          const next = new Map(state.tickets)
        ...
        try {
          ...
          const results = await Promise.all(
            projectIds.map(async (pid) => ({
              projectId: pid,
              tickets: await kanban.ticket.getByProject(pid, includeArchived(pid))
            }))
          )
          const diagnosticsByProject = await Promise.all(
            results.map(async (result) => ({
              projectId: result.projectId,
              diagnostics: await kanban.diagnostics.get(result.projectId).catch(() => [])
            }))
          )
