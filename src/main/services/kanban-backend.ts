...
export interface KanbanBackend {
  ...
  createBatch(projectId: string, data: KanbanTicketBatchCreate): Promise<KanbanTicketBatchCreateResult>
  update(projectId: string, ticketId: string, data: KanbanTicketUpdate): Promise<KanbanTicket | null>
  move(
    projectId: string,
    ticketId: string,
    column: KanbanTicketColumn,
  ...
  exportBoard(projectId: string): Promise<{ tickets: KanbanTicket[]; dependencies: TicketDependency[] }>
  importTickets(
    projectId: string,
    tickets: Array<{
      id: string
...
class InternalKanbanBackend implements KanbanBackend {
  ...
  async createBatch(projectId: string, data: KanbanTicketBatchCreate): Promise<KanbanTicketBatchCreateResult> {
  ...
  async update(projectId: string, ticketId: string, data: KanbanTicketUpdate): Promise<KanbanTicket | null> {
  ...
  async addTokens(projectId: string, ticketId: string, tokens: number): Promise<KanbanTicket | null> {
    const existing = await this.get(projectId, ticketId)
    if (!existing) return null
  ...
  async attachPR(projectId: string, ticketId: string, prNumber: number, prUrl: string): Promise<void> {
  ...
  async removeDependency(projectId: string, dependentId: string, blockerId: string): Promise<boolean> {
    const dependent = await this.get(projectId, dependentId)
  ...
  async exportBoard(projectId: string): Promise<{ tickets: KanbanTicket[]; dependencies: TicketDependency[] }> {
    return {
...
class MarkdownKanbanBackend implements KanbanBackend {
  ...
  async list(projectId: string, includeArchived: boolean): Promise<KanbanTicket[]> {
    ...
    return index.tickets
      .filter((ticket) => includeArchived || !ticket.archived_at)
      .sort(ticketSort)
  }
  ...
  async create(projectId: string, data: KanbanTicketCreate): Promise<KanbanTicket> {
    ...
    await this.writeRuntime(projectId, id, {
  ...
  async createBatch(projectId: string, data: KanbanTicketBatchCreate): Promise<KanbanTicketBatchCreateResult> {
    ...
    for (const draft of drafts) {
      ...
      for (const blockerDraftKey of draft.depends_on) assertNonEmptyString(blockerDraftKey, 'Dependency draft key')
    }
    ...
    for (const draft of drafts) {
      ...
      if (ids.has(id)) {
        throw new Error(`Cannot create markdown ticket "${id}" because that card id is already used in this batch.`)
      }
    ...
    try {
      ...
      for (const draft of drafts) {
        ...
        if (!dependent) throw new Error(`Created ticket for draft "${draft.draft_key}" could not be loaded`)
        for (const blockerDraftKey of draft.depends_on) {
          const blocker = byDraftKey.get(blockerDraftKey)
          if (!blocker) throw new Error(`Created ticket for draft "${blockerDraftKey}" could not be loaded`)
  ...
  async update(projectId: string, ticketId: string, data: KanbanTicketUpdate): Promise<KanbanTicket | null> {
    ...
    if (data.goal_success_criteria !== undefined) publicUpdates.goal_success_criteria = data.goal_success_criteria

    if (data.attachments !== undefined) runtimeUpdates.attachments = data.attachments
    if (data.current_session_id !== undefined) runtimeUpdates.current_session_id = data.current_session_id
    ...
    if (data.pending_launch_config !== undefined) runtimeUpdates.pending_launch_config = data.pending_launch_config
  ...
  async archive(projectId: string, ticketId: string): Promise<KanbanTicket | null> {
    await this.update(projectId, ticketId, { archived_at: new Date().toISOString() } as KanbanTicketUpdate)
  ...
  async addDependency(
    ...
    if (dependentId === blockerId) return { success: false, error: 'A ticket cannot depend on itself' }
    const index = await this.reloadIndex(projectId)
  ...
  async getDependents(projectId: string, ticketId: string): Promise<KanbanTicket[]> {
    ...
    return index.tickets.filter((ticket) => {
      ...
      return card ? readDependencies(card.frontmatter, ticket.created_at).some((dep) => dep.blocker_id === ticketId) : false
    })
  }
  ...
  async importTickets(
    ...
    const selectedIds = new Set(tickets.map((ticket) => ticket.id))
    const index = await this.reloadIndex(projectId)
    const duplicateId = [...selectedIds].find((ticketId) => (index.pathsById.get(ticketId) ?? []).length > 1)
    if (duplicateId) {
      throw new Error(`Cannot import ticket "${duplicateId}" because that markdown card id is duplicated. Resolve duplicate markdown IDs before importing.`)
    }
    ...
    for (const folder of folders) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          const title = asString(frontmatter.title) ?? titleFromBody(parsed.body) ?? basename(filePath, extname(filePath))
  ...
  private async reloadIndex(projectId: string): Promise<MarkdownIndex> {
    ...
    const index: MarkdownIndex = { projectId, tickets, cardsById, pathsById, diagnostics, loadedAt: Date.now() }
  ...
  private async requireMutableCard(projectId: string, ticketId: string): Promise<ParsedMarkdownCard> {
    const index = await this.reloadIndex(projectId)
  ...
  private requireMutableCardFromIndex(index: MarkdownIndex, ticketId: string): ParsedMarkdownCard {
    const duplicate = index.diagnostics.find((d) => d.ticketId === ticketId && d.kind === 'duplicate_id')
  ...
  private assertCardIdAvailableInIndex(index: MarkdownIndex, ticketId: string): void {
    if (index.cardsById.has(ticketId) || (index.pathsById.get(ticketId) ?? []).length > 0) {
      throw new Error(`Cannot create markdown ticket "${ticketId}" because that card id already exists.`)
    }
  }
  ...
  private async writeRuntime(
    ...
    getDatabase()
      .getRawDb()
      .prepare(`UPDATE markdown_kanban_card_state SET ${updates.join(', ')} WHERE project_id = ? AND card_id = ?`)
      .run(...values)
  }
  ...
  private cleanupRuntimeRows(
    ...
    for (const row of rows) {
      ...
      if (row.orphaned_at) {
        db.prepare('DELETE FROM markdown_kanban_card_state WHERE project_id = ? AND card_id = ?').run(projectId, row.card_id)
      } else {
        db.prepare(
  ...
  private async nextSortOrder(projectId: string, column: KanbanTicketColumn): Promise<number> {
    const tickets = await this.list(projectId, false)
    const max = Math.max(-1, ...tickets.filter((ticket) => ticket.column === column).map((ticket) => ticket.sort_order))
    return max + 1
  }
}
...
export async function setKanbanStorageMode(
  ...
  if (internalTickets.length > 0) {
    return {
      success: false,
      error: 'Changing Kanban storage mode is only supported for projects with no active or archived internal cards.'
    }
  }
  if (currentMode === 'markdown' && await markdownBackend.hasAnyCardLikeFiles(projectId)) {
    return {
      success: false,
      error: 'Changing Kanban storage mode is only supported after clearing markdown card files from the current board folders.'
    }
  }
...
async function preflightMarkdownLayoutMoves(moves: MarkdownLayoutMove[]): Promise<void> {
  ...
  for (const move of moves) {
    ...
    if (previousSource && previousSource !== move.source) {
      throw new Error(`Cannot change Kanban folder layout because multiple cards would move to "${move.target}"`)
    }
...
export async function detachWorktreeFromAllKanbanBackends(worktreeId: string): Promise<number> {
  return (await internalBackend.detachWorktree(worktreeId)) + (await markdownBackend.detachWorktree(worktreeId))
}

export async function syncPRToAllKanbanBackends(worktreeId: string, prNumber: number, prUrl: string): Promise<void> {
...
function createdAtFromStat(fileStat: { birthtime: Date; birthtimeMs: number }, fallback: string): string {
...
function validateKnownFrontmatter(frontmatter: Frontmatter, ticketId: string | null): void {
  const invalid = (field: string, expected: string): never => {
    throw new MarkdownCardError('invalid_frontmatter', ticketId, `Invalid ${field}; expected ${expected}`)
  }
  ...
  if ('archived_at' in frontmatter && frontmatter.archived_at !== null && typeof frontmatter.archived_at !== 'string') {
    invalid('archived_at', 'a string or null')
  }
  for (const field of ['external_provider', 'external_id', 'external_url', 'github_pr_url', 'mark', 'goal_success_criteria']) {
    const value = frontmatter[field]
    if (field in frontmatter && value !== null && typeof value !== 'string') invalid(field, 'a string or null')
  }
  if ('mark' in frontmatter && frontmatter.mark !== null && !asMark(frontmatter.mark)) {
    invalid('mark', 'common, rare, epic, legendary, or null')
  }
  if ('github_pr_number' in frontmatter && frontmatter.github_pr_number !== null && asNumber(frontmatter.github_pr_number) === null) {
    invalid('github_pr_number', 'a finite number or null')
  }
  if ('goal_mode' in frontmatter && frontmatter.goal_mode !== null && asBoolean(frontmatter.goal_mode) === null) {
...
async function writeMarkdownFile(filePath: string, frontmatter: Frontmatter, body: string): Promise<void> {
...
function wouldCreateDependencyCycle(deps: TicketDependency[], dependentId: string, blockerId: string): boolean {
...
async function uniquePath(folder: string, fileName: string, reservedPaths: Set<string> = new Set()): Promise<string> {
...
function asMode(value: unknown): KanbanTicket['mode'] {
  return typeof value === 'string' && VALID_MODES.has(value) ? (value as KanbanTicket['mode']) : null
}

function asMark(value: unknown): KanbanTicket['mark'] {
  return typeof value === 'string' && VALID_MARKS.has(value) ? (value as KanbanTicket['mark']) : null
}
