...
const { fakeWatchers, watchMock, mockDatabase, mockState } = vi.hoisted(() => {
  ...
  return { fakeWatchers, watchMock, mockDatabase, mockState }
})
...
describe('MarkdownKanbanWatcher', () => {
  ...
  test('debounces markdown candidate file changes into one project event', async () => {
    ...
    expect(send).toHaveBeenCalledWith('kanban:markdown:changed', {
      projectId: 'project-1',
