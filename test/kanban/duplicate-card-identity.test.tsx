...
describe('duplicate markdown card renderer identity', () => {
  ...
  test('duplicate cards get distinct DOM and layout identities while retaining blocking diagnostics', () => {
    ...
    expect(ticketKeys.every((key) => key?.startsWith(`${ticketKey('proj-1', 'shared')}:duplicate:`))).toBe(true)
  ...
  test('cross-column drops compute sort order from the dragged project only', async () => {
    const tickets = [
      makeTicket({ id: 'done-a', project_id: 'proj-1', title: 'Project 1 Done', column: 'done', sort_order: 10 }),
      makeTicket({ id: 'done-b', project_id: 'proj-2', title: 'Project 2 Done', column: 'done', sort_order: 100 })
    ]
    const { container } = render(
      <TooltipProvider>
  ...
  test('same-column reorder filters the dragged project key before computing sort order', async () => {
    const tickets = [
      ...
      makeTicket({ id: 'shared', project_id: 'proj-2', title: 'Project 2 Shared', sort_order: 100 }),
